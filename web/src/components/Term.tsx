...
export function Block({ index, label, title, intro, children }: BlockProps) {
  const ref = useReveal()
  return (
...
export function Bar({ label, fill, note, accent }: { label: string; fill: number; note?: string; accent?: boolean }) {
  const total = 16
